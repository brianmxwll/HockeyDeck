...
class ParsePlays:
    def __init__(self):
        self.frequency = 5 #Value in seconds, how often this should be run.
    ...
    def parse(self):
        ...
        plays = self.getAllPlaysForDate(today)

        jPlays = json.dumps(plays)

        f = open('G:/HockeyDeck/plays.json','w')
        f.write(jPlays)
        f.close()

    def getAllPlaysForDate(self, date):
        #First get all of the game ids that we care about.
        sched = misc.getPickleData('G:/HockeyDeck/python/schedule.p') #Dict = date -> [gameid : matchup]
        
        gameids = sched[date].keys()

        #Master list of plays.
        allPlays = misc.getPickleData('G:/HockeyDeck/python/temp/plays' + date + '.p')
        ...
        for gameid in gameids:
            plays = self.getPlaysOnPage(gameid)
            if plays is not None:
                ...
                for key, play in plays.iteritems():
                    if key not in allPlays: #If we haven't seen this play, add it at the top
                        play['meta'] = self.calcPlayMeta(play.copy())
                        allPlays.prepend(key, play)
                    else: #If we HAVE seen it, check to see if it has changed.
                        oldPlay = self.stripPlay(allPlays[key].copy())
                        diff = [k for k in oldPlay if oldPlay[k] != play[k]]
                        #diff = set(oldPlay.values()) ^ set(self.stripPlay(play.copy()).values()) #Find any new items
                        if len(diff) > 0: #Something changed, update.
                            play['meta'] = self.calcPlayMeta(play)
        ...
        #    allPlays[key]['meta'] = self.calcPlayMeta(allPlays[key].copy(), True)
            
        #All done with the plays, pickle the data for later use.
        misc.savePickleData('G:/HockeyDeck/python/temp/plays' + date + '.p', allPlays)

        #Return the plays, sorted by insertion date.
        return allPlays.values()

    def getPlaysOnPage(self, gameid):
        ...
        for row in rows:
            ...
            vals['time'] = self.cleanTime(cells[3].getText())
    ...
    def stripPlay(self, original):
        if 'meta' in original:
            del original['meta']
        return original

    def calcPlayMeta(self, play, recalc=False):
        meta = {}
        ...
        if play['event'] in ['BLOCK', 'FAC', 'GIVE', 'GOAL', 'HIT', 'MISS', 'PENL', 'SHOT', 'TAKE', 'CHL']:
            meta['mainLogo'] = self.cleanTeamName(play['desc'].split(' ')[0])
            meta['actionPlayers'] = self.getActionPlayers(play)
    ...
    def cleanTime(self, txt): #0:0020:00 -> 20:00
        sep = txt.find(':')
        return txt[sep+3:]

    def getActionPlayers(self, play):
